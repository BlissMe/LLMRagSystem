{"id":"49203","variant":"standard","title":"Therapy Agent with Full Progress Tracking"}
...
"""
    ...
    if is_therapy_suggested:
        send_monitor_event(
            "THERAPY_SUGGESTED",
            {"input": {"user_query": data.user_query}, "output": {"therapy_id": therapy_id, "therapy_name": therapy_name, "therapy_path": therapy_path}},
            data.user_id,
            data.session_id
        )
    ...
    if action_detected:
        save_therapy_history(db, data.user_id, data.session_id, therapy_name, therapy_id)
        send_monitor_event(
            "THERAPY_STARTED",
        ...
        # Optional: send intermediate updates (25%, 50%, etc.)
